"""
Docstring for 2026.january.valid_pawn_moves
Link to problem: https://www.freecodecamp.org/learn/daily-coding-challenge/2026-01-30
Valid Pawn Moves
...
def find_pawn_moves(position):
    pawn_col = position[0]
    pawn_row = int(position[1])
    moves = []
    if pawn_row == 2:
        moves.append(f"{pawn_col}{pawn_row + 1}")
        moves.append(f"{pawn_col}{pawn_row + 2}")
    elif pawn_row < 8:
        moves.append(f"{pawn_col}{pawn_row + 1}")
    return moves

print(find_pawn_moves("D4")) # should return ["D5"].
print(find_pawn_moves("B2")) # should return ["B3", "B4"].
print(find_pawn_moves("A7")) # should return ["A8"].
print(find_pawn_moves("G2")) # should return ["G3", "G4"].
print(find_pawn_moves("E3")) # should return ["E4"].