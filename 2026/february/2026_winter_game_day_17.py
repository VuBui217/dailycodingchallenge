"""
Link to problem: https://www.freecodecamp.org/learn/daily-coding-challenge/2026-02-22
...
def count_medals(winners):
    ...
    for i in range(len(winners)):
        for j in range(3):
            if winners[i][j] not in medals_map:
                medals_map[winners[i][j]] = [0] * 3
            medals_map[winners[i][j]][j] += 1
    
    rows = []
    for country, medals in medals_map.items():
        rows.append((country, medals[0], medals[1], medals[2], sum(medals)))

    rows.sort(key=lambda x: (-x[1], x[0]))

    output = ['Country,Gold,Silver,Bronze,Total']
    for country, gold, silver, bronze, total in rows:
        output.append(f'{country},{gold},{silver},{bronze},{total}')
